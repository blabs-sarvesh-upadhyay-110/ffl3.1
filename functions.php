...
function enqueue_custom_script() {
    // Enqueue Crypto.js from CDN
    wp_enqueue_script('crypto-js', 'https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.2.0/crypto-js.min.js', array(), null, false);

    // Get the URL to the theme directory
    $theme_url = get_template_directory_uri();

    // Enqueue custom script
    wp_enqueue_script('custom-script', $theme_url . '/includes/js/custom-script.js', array('jquery', 'crypto-js'), '1.0', true);

    // Retrieve $api_url and $secret_key from ACF options page
    $encryption_key = get_field('encryption_key', 'option');
    $api_url = get_field('api_url', 'option');

    // Localize custom script with API settings
    wp_localize_script('custom-script', 'apiSettings', array(
